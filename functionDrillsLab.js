...
//CODE HERE
let compareNums = (num1, num2) => num1 > num2 ? num1 : num2 || num1
...
function faveColorFinder(color){
  switch(color !== null){
    case color.toLowerCase() == 'red':
      return `red is a great color`
      break;
    case color.toLowerCase() == 'green':
      return `green is a solid favorite color`
      break;
    case color.toLowerCase() == 'black':
      return `so trendy`
      break;
    default:
...
//CODE HERE
let thatsOdd = (num) => num % 2 == 0 ? `That's not odd!` : `That is odd indeed!`
...
//CODE HERE
const makeUpper = (str) => console.log(str.toUpperCase())
...
function emailCheck(email){
  let converted = String(email).trim()
...
function brainWallet(gold){
  return Math.floor(gold / 3)
}

const totalFrogs = brainWallet(gold)
...
//CODE HERE
const totalFrogs2 = brainWallet(gold)
...
//CODE HERE
// I had to look at the MDN for .every(), as I haven't used it before but, this worked when I tried it, hope it does for you too.
// basically .every takes a predecessor and an index, and just checks back on the previous element, unless there was no element before it.
// it also performs the function test on each element in the array except for the first one, because it evaluates the index as being 0

function ascender(arr) {
  return arr.every(function (x, i) {
      return i == 0 || x >= arr[i - 1];
  });
}
let arrayIsAscending = ascender(sampleArray);
